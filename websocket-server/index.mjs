// Serveur WebSocket standalone pour le jeu Menteur
...
// Configuration CORS - Accepte votre frontend Vercel
...
const io = new Server(httpServer, {
  cors: {
    origin: (origin, callback) => {
      // Accepter les requêtes sans origin (comme les health checks)
      if (!origin) return callback(null, true)
      // Accepter les origines autorisées ou toutes si FRONTEND_URL contient *
      if (process.env.FRONTEND_URL === '*' || ALLOWED_ORIGINS.includes(origin)) {
